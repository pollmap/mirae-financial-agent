...
`100문항`은 팀의 live E2E release gate이다. 이를 README·제안서·발표에서 공식 수치처럼
말하면 안 된다.
...
| API/재질문 | 5-field schema, bounded options, signed follow-up, 변조/누락 token | ASGI contract 회귀 |
...
| 실제 HCX | 20 planner one-vs-two parity, 100 two-stage planner→evidence→independent SQL oracle | 스크립트 완비, key 주입 전에는 **미실행** |
...
- 전체 `pytest`는 **272/272**, Ruff는 `app deploy etl scripts tests eval` 전체 통과,
  runtime compliance는 **91 files / 0 findings**이다. 독립 oracle 640/640,
...
저장하지 않으며 production preflight는 둘 다 없으면 실패한다.
...
1. 팀이 발급한 HCX key/model/endpoint로 20 parity와 100 E2E report를 실제 생성한다.
